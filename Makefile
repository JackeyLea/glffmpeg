...
PROGTEST2=glffmpeg_example$(EXESUF)
...
all: lib $(PROGS_G) $(PROGS) $(PROGTEST) $(PROGTEST2) $(VHOOK) $(QTFASTSTART) $(DOC)
...
#glffmpeg_example$(EXESUF): glffmpeg_example.o .libs
#	$(CC) $(LDFLAGS) -I$(SRC_PATH)/glffmpeg/ -L$(SRC_PATH)/glffmpeg/ -lglffmpeg -o $@ glffmpeg_example.o $(EXTRALIBS)

glffmpeg_example: glffmpeg_example.cpp
	g++ glffmpeg_example.cpp -I$(SRC_PATH)/glffmpeg/ -L$(SRC_PATH)/glffmpeg/ -lglffmpeg -lGL -o glffmpeg_example
...
ifeq ($(CONFIG_PP),yes)
	...
	$(MAKE) -C glffmpeg    install-headers
...
clean:
	...
	$(MAKE) -C glffmpeg    clean
	$(MAKE) -C tests       clean
	$(MAKE) -C vhook       clean
	$(MAKE) -C doc         clean
	rm -f glffmpeg_example *.mov *.o *.d *~ .libs gmon.out TAGS \
